...
lab.experiment("registration", function () {
    ...
    lab.test("Send Invite", function(done) {
        ...
        registration.sendInvite(email).delay(500).then(function(info) {
            
            Code.expect(info).to.not.be.undefined();
            Code.expect(info).to.not.be.null();
            Code.expect(info.accepted).to.not.be.undefined();
            Code.expect(info.accepted).to.not.be.null();
            Code.expect(info.accepted.length).to.be.above(0);
